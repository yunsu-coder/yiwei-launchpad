  ...
  for (const url of urls) {
    try {
      ...
      if (url.includes('music.163.com')) {
        try {
          const netease = await scrapeNetease(url);
          if (netease && netease.buf) {
            const imgDir = path.join(sessionDir, 'images');
            fs.writeFileSync(path.join(imgDir, netease.name), netease.buf);
            result.images.push({ name: netease.name, size: netease.size, url: netease.url });
            result.imageCount++;
            if (netease.title) result.title = netease.title;
          }
        } catch(e) {
          result.errors.push({ url, error: '网易云: ' + e.message });
          result.errorCount++;
        }
        continue; // 跳过常规 HTML 抓取
      }
...
async function scrapeNetease(urlStr) {
  // 提取歌曲 ID: music.163.com/song?id=123456 或 /song/123456
  let songId = null;
  const idMatch = urlStr.match(/[?&]id=(\d+)/);
  const pathMatch = urlStr.match(/music\.163\.com\/song\/(\d+)/);
  songId = idMatch?.[1] || pathMatch?.[1];
  ...
  try {
    ...
    const [urlRes, detailRes] = await Promise.all([
      song_url({ id: songId }),
      song_detail({ ids: songId }),
    ]);

    const songData = urlRes.body?.data?.[0];
    const songInfo = detailRes.body?.songs?.[0];
    const title = songInfo?.name || '未知歌曲';
    const artist = (songInfo?.ar || []).map(a => a.name).join('/');

    if (!songData?.url) return null; // 无可用音频

    // 下载音频
    const buf = await downloadImage(songData.url, 30000, 'https://music.163.com/');
    if (!buf || buf.length < 10240) return null;

    const safeTitle = safeName(title + (artist ? '_' + artist : '')).slice(0, 60);
    const name = 'netease_' + safeTitle + '.mp3';
    return { name, url: songData.url, buf, size: buf.length, title: title + ' - ' + artist };
  } catch (e) {
    return null;
  }
}
...
module.exports = {
  ...
  scrapeBilibili, scrapeArchiveOrg, captureMediaWithBrowser, scrapeNetease,
