...
async function scrapeTieba(kw, opts = {}) {
  ...
  try {
        ...
        const pageThreads = await page.evaluate(() => {
          ...
          if (threadLinks.length > 0) {
            ...
            threadLinks.forEach(a => {
              ...
              // 从全文提取作者名（格式: "AuthorName  贴吧成长等级...回复于... title"）
              ...
              results.push({ tid: m[1], title, author, replyCount, href: href.startsWith('http') ? href : 'https://tieba.baidu.com' + href });
            });
    ...
    // 爬取帖子详情（内容 + 评论）—— 仅当需要评论时才访问详情页
    toFetch = includeComments ? threads.slice(0, maxThreads) : [];
    for (let i = 0; i < toFetch.length; i++) {
      ...
      try {
        ...
        t.detailTitle = (detail.title || t.title).replace(/[-_].*$/, '').trim(); // 清理 URL 片段
      } catch (e) {
        t.posts = [];
...
function formatTiebaText(kw, threads, includeComments) {
  ...
  lines.push('  帖子数量：' + threads.length + (includeComments ? '  |  含评论内容' : '  |  快速模式（仅标题，勾选「含评论」可获取内容）'));
  lines.push('='.repeat(60));
  lines.push('');
