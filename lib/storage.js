...
function listFiles(dirRel = '', opts = {}) {
  const dir = path.join(FILES_DIR, dirRel || '');
  if (!fs.existsSync(dir)) return [];
  const all = scanDir(dir, FILES_DIR);
  // 扁平列表：返回所有文件和文件夹
  const results = [];
  const seen = new Set();
...
function deleteFile(name) {
  ...
  // 移到回收站而不是直接删除
  const trashPath = path.join(TRASH_DIR, Date.now() + '_' + path.basename(name));
...
function getFilePath(name) {
  // Try flat first, then try with subdirectory paths
  let fp = path.join(FILES_DIR, name);
  if (fs.existsSync(fp) && !fs.statSync(fp).isDirectory()) return fp;
  // Search recursively
  const found = scanDir(FILES_DIR).filter(f => !f.isDir && f.name === name);
